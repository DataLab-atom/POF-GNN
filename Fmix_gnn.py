...
from utils import load_data, accuracy,load_pokec, fair_metric, dp_regularizer, fair_mixup
...
parser.add_argument('--prefix', type=str, default='Fmix')
...
parser.add_argument('--num_gnn_layer', type=int, default=3, help='number of gnn layers')
parser.add_argument("--lambda2", type=float, default=9, help="Teleport Probability")

parser.add_argument('--hyper_reg', type=float, default=0.0, help='fairness regularizer')
...
for run_time in range(RUNNING_TIME):
    ...
    for epoch in range(args.epochs):
        ...
        # cls_loss += args.hyper_reg * dp_regularizer(all_y[:, 1], labels, sens, idx_train)
        cls_loss += args.hyper_reg * fair_mixup(all_logit[idx_train], train_labels.long(), sens[idx_train], model)
