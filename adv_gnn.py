...
def pretrain_adversary(adv, clf, features, labels, sens, idx_train, adv_optimizer, adv_criterion, epochs):
    # print(f'pretrain_adversary')
    for epoch in range(epochs):
        all_logit = clf(features)
        all_y = F.softmax(all_logit, dim=1)

        adv.zero_grad()
        # print(f'p_y={p_y}')
        p_z = adv(all_y).flatten()
        # print(f'p_z={p_z}')
        # print(f'z={z}')
        loss = adv_criterion(p_z[idx_train], sens[idx_train]) * args.hyper_reg
        loss.backward()
        adv_optimizer.step()
    return adv

def pretrain_classifier(clf, features, labels, sens, idx_train, clf_optimizer, clf_criterion, epochs):
    # print(f'pretrain_classifier')
    for epoch in range(epochs):
        ### inference
        ...
        ### training loss
        cls_loss = clf_criterion(all_logit[idx_train],train_labels.long())
...
parser.add_argument('--prefix', type=str, default='adv')
...
parser.add_argument('--hyper_reg', type=float, default=0.0, help='adv hyper')
...
for run_time in range(RUNNING_TIME):
    ...
    if args.model=="APPNP":
        log_path = f'log/{args.dataset}/{args.prefix}/{args.model}/lambda2={args.lambda2}/hyper_reg={args.hyper_reg}'
    else:
        log_path = f'log/{args.dataset}/{args.prefix}/{args.model}/hyper_reg={args.hyper_reg}'
    ...
    ### pretrain classifier
    N_CLF_EPOCHS = 70
    clf = pretrain_classifier(clf, features, labels, sens_train, idx_train, \
                            clf_optimizer, clf_criterion, N_CLF_EPOCHS)


    ### pretrain adversary
    # lambdas = torch.Tensor([130, 30])
    adv = Adversary(2, 1).to(device)
    adv_criterion = nn.MSELoss()
    adv_optimizer = optim.Adam(adv.parameters(), lr=args.lr)
    N_ADV_EPOCHS = 30
    adv = pretrain_adversary(adv, clf, features, labels, sens_train, idx_train, \
                    adv_optimizer, adv_criterion, N_ADV_EPOCHS)
    ...
    for epoch in range(args.epochs):
        ...
        ### train adv
        adv_epochs = 3
        for epoch_adv in range(adv_epochs):
            all_logit = clf(features)
            all_y = F.softmax(all_logit, dim=1)
            ...
            # print(f'p_y={p_y}')
            p_z = adv(all_y).flatten()
            loss_adv = adv_criterion(p_z[idx_train], sens_train[idx_train]) * args.hyper_reg
            loss_adv.backward()
            adv_optimizer.step()
        ...
        ### training loss
        clf_loss = clf_criterion(all_logit[idx_train],train_labels.long())
        clf_loss -= loss_adv
