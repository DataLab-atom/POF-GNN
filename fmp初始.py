...
class FMP(GraphConv):
    ...
    def __init__(self, 
        ...
        self.score = nn.Parameter(torch.ones(1, 1))
        self.score = nn.hypernet()
    ...
    def emp_forward(self, g, x, hh, K, sen):
        ...
        for _ in range(K):
            ...
            if lambda1 > 0:
                # print(f'sen={sen.shape}')  sen=torch.Size([1, 66569])
                # print(f'y={y.shape}')  y=torch.Size([66569, 2])
                # exit()
                ...
                # z = sen @ F.softmax(y, dim=1) / gamma * self.score



                # # x_bar = y - gamma * self.score
