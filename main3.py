...
# Load german dataset
    ...
    adj, features, labels, idx_train, idx_val, idx_test, sens, idx_sens_train = load_german(args.dataset, sens_attr,
                                                                                            predict_attr,
                                                                                            path=path_german,
                                                                                            label_number=label_number,
                                                                                            )
    edge_index, _ = from_scipy_sparse_matrix(adj)  # [0]
    # norm_features = feature_norm(features)
    # norm_features[:, sens_idx] = features[:, sens_idx]
    # features = norm_features
    lr = .01
...
for run_time in range(RUNNING_TIME):
    ...
    for epoch in range(args.epochs):
        ...
        for i in range(epoch_d):
            ...
            ### training loss
            # print(all_y[idx_train])
            # print(train_labels.long())
            # cls_loss = criterion(all_y[idx_train],train_labels.long())
            lossd = criterion(all_y, features[:, 0].long())
            ...
            losses = torch.stack((alapha * lossd, beta * parity_loss, beta * equality_loss))
        ...
        for i in range(epoch_c):
            ...
            loss2 = F.cross_entropy(
                all_y[idx_train], train_labels.long())
        ...
        logger.info('D_EO: {:.4f}, val D_EO: {:.4f}, test D_EO: {:.4f},F1: {:.4f}' \
                    .format(eo_train, eo_val, eo,f1_test))
        eo_list.append(eo)
        sp_list.append(parity)
        f1_list.append(f1_test)

    #eo_list_plt = [t.cpu().detach().numpy() for t in eo_list]
    #sp_list_plt = [t.cpu().detach().numpy() for t in sp_list]
    #f1_list_plt = [t.cpu().detach().numpy() for t in f1_list]
    ...
    #plt.scatter(f1_list, eo_list, cmap='viridis')
    plt.plot(f1_list, eo_list, marker='o', linestyle='-', color='b', label='EO List')
    plt.show()
    """
    plt.figure(figsize=(8, 6))
