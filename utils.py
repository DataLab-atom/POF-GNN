...
def load_pokec(dataset,sens_attr,predict_attr, path="dataset/pokec/", train_ratio=0.8,seed=19,test_idx=False):
    """Load data"""
    ...
    if dataset == 'nba':
        # 创建一个布尔数组，表示每行是否包含整数值
        contains_int = np.all(np.isnan(edges_unordered.astype(int)), axis=1)

        # 保留包含整数值的行
        edges_unordered = edges_unordered[contains_int]
...
def load_pokec_sub(dataset,sens_attr,predict_attr, path="dataset/pokec/", train_ratio=0.8,seed=19,test_idx=False):
    """Load data"""
    print('Loading {} dataset from {}'.format(dataset,path))
...
class BestResultTracker:
    def __init__(self):
        self.best_acc_test = 0  # 最大化acc_test
        self.best_parity = float('inf')  # 最小化parity
        self.best_eo = float('inf')  # 最小化eo
    ...
    def update(self, acc_test, parity, eo):
        # 更新逻辑：需要同时考虑三个指标
        # 这里我们假设优先级是 acc_test > parity > eo
        better_acc_test = acc_test > self.best_acc_test
        better_or_equal_parity = parity < self.best_parity
        better_or_equal_eo = eo < self.best_eo

        if better_acc_test or (acc_test == self.best_acc_test and better_or_equal_parity and better_or_equal_eo):
            self.best_acc_test = acc_test
            self.best_parity = parity
            self.best_eo = eo
            self.best_overall_result = (acc_test, parity, eo)
            print(f"New best result found: Acc={acc_test}, Parity={parity}, EO={eo}")

    def get_best_results(self):
        acc_test, parity, eo = self.best_overall_result[0],self.best_overall_result[1],self.best_overall_result[2]
        ...
        acc_test_mean = np.mean(self.acc_test_list)
        acc_test_var = np.var(self.acc_test_list)
        parity_mean = np.mean(self.parity_list)
        parity_var = np.var(self.parity_list)
        eo_mean = np.mean(self.eo_list)
        eo_var = np.var(self.eo_list)

        print(f'acc_test: Mean={acc_test_mean}, Variance={acc_test_var}')
        print(f'parity: Mean={parity_mean}, Variance={parity_var}')
        print(f'eo: Mean={eo_mean}, Variance={eo_var}')

        # return self.best_overall_result